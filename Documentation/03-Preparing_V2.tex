...
\section{Installing rama}
\texttt{rama} is a R package  available at \url{https://github.com/r-and-gama/rama}.
...
\begin{lstlisting}
setup_gama("U:/Users/Programmes extra/GAMA1.8_RC2_EmbeddedJDK_Win_64bits")
\end{lstlisting}


\section{Preparing an experiment}

An experiment is defined as a set of simulations each with various parameter values.

\subsection{Reading the model}

Now, one must read the model (here \texttt{snubbies\_V2.gaml}) and extract informations that will be used by R. The path to the model must be declared explicitely (here \texttt{snubbies\_V2.gaml}  is in the folder \texttt{"U:/Users/pgiraudo2/gama\_workspace/snubbies/models")}. Again this to adapt to your own computer.

\begin{lstlisting}
modpar<-get_parameters_names("U:/Users/pgiraudo2/gama_workspace/snubbies/models/snubbies_V2.gaml")
modpar
\end{lstlisting}
...
A data.frame must be prepared with each row the parameter values of each simulation.
...
Note that parameters can be explored randomly allocating values between limits. See for instance the function \texttt{runif} in R (but many others are available).

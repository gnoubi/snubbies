...
global
{
	...
	float max_snubby_mortality_hour <- 0.0;
	...
	float viscosity_init_habitat_5 <- 0.0001;
	...
	float security_init_habitat_5 <- 0.0001;
	...
	init
	{
		step <- 1 #hour;
		death <- 0 ;
		max_snubby_mortality_hour<- 1-convert_probability_from_year_to_hour(max_snubby_survival_init);
...
species Snubby  skills:[moving]
{
	...
	reflex be_killed when:is_exploring
	{
		...
		else
		{
			...
			float probability_to_die <- max_snubby_mortality_hour/local_group.security;
			if(local_group.security>1.0) {local_group.security<-1;}

...
global
{
	...
	float long_perception_distance <- 6#km; 
	float short_perception_distance <- long_perception_distance / 3 ;
	float angle <- 45.0;
...
species world_boundaries schedules:[]
{
	aspect base
	{
...
species Snubby_group schedules:[]
{
	int id;
	string name;
	int init_population;
...
species Snubby  skills:[moving]
{
	...
	//upgrade for the new mobility behavior
	point my_longterm_target <- nil;
	point my_shortterm_target <- nil;
	
	geometry  my_perception <- nil;
	

	reflex choose_long_term_target when: is_exploring and my_longterm_target = nil
	{
		geometry my_perception_area <- circle(long_perception_distance);
		habitats choosen_habitat <- one_of(habitats where(each.DN >= 4 and each overlaps my_perception_area));
		my_longterm_target <- any_location_in(choosen_habitat);
	}
	...
	reflex choose_short_term_target when: is_exploring and my_longterm_target != nil and my_shortterm_target = nil
	{
		...
		geometry geom <- polygon([location,p1,p2,location ]);
		
		current_perception <- current_perception inter(geom);	
		
		if(my_perception = nil) {
			my_perception <- current_perception;
		}
		else
		{
			my_perception <- current_perception - my_perception;
		}
		habitats choosen_habitat <- one_of(habitats where(each.DN >= 2 and each overlaps my_perception));
		my_shortterm_target <- any_location_in(choosen_habitat);
	}
	
	reflex walk2 when: my_shortterm_target != nil
	{
		habitats local_habitat <- habitats closest_to self.location; // //one_of(habitats overlapping(self.location));
		do goto  target:my_shortterm_target speed:local_habitat.viscosity*max_snubby_speed on:world_boundaries_shape recompute_path:false;
		if(location = my_shortterm_target)
		{
	...
	reflex be_killed when:is_exploring
	{
		...
		if(local_habitat!=nil)
		{
			...
			if(probability_to_die>1) {probability_to_die<-1.0;} // truncating to 1 is not and optimal solution
